...
\textcolor{red}{$\star$ Question 1:} For the six-sided fair die, what is $f_k$ if $k=7$? $k=1.5$?

<br> \hfill\break

Related to the pmf is the *cumulative distribution function* (cdf), $F(x)$. \[ F(x) \equiv \text{Pr}(X \leq x) \]

For the 6-sided die \[ F(x)= \sum\_{k=1}\^x f_k \] where $x \in 1\dots 6$.

<br> \hfill\break

\textcolor{red}{$\star$ Question 2:} For the fair 6-sided die, what is $F(3)$? $F(7)$? $F(1.5)$?
...
\textcolor{red}{$\star$ Question 5:} Properites of Expected Values
...
\textcolor{red}{$\star$ Question 6:} Functions of Random Variables
...
\[ \bar{\theta} \pm t\_{n-p,\alpha/2} \times s ;;;;; \Leftrightarrow ;;;;; \[\bar{\theta}- t\_{n-p,\alpha/2} \times s, \bar{\theta} + t\_{n-p,\alpha/2} \times s\] \]
